...
class GuideAvailability(models.Model):
    ...
    guide_service = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.CASCADE,
        related_name="guide_availabilities",
    )
    ...
    def __str__(self):
        ...
        return (
            f"{self.guide.get_full_name() or self.guide.username}: "
            f"{status} {self.start:%Y-%m-%d %H:%M} → {self.end:%Y-%m-%d %H:%M}"
        )
...
class GuideAvailabilityShare(models.Model):
    ...
    guide_service = models.ForeignKey(
        "orgs.GuideService",
        on_delete=models.CASCADE,
        related_name="availability_shares",
    )
    visibility = models.CharField(
        max_length=20,
        choices=GuideAvailability.VISIBILITY_CHOICES,
        default=GuideAvailability.VISIBILITY_BUSY,
    )
...
class GuideCalendarIntegration(models.Model):
    ...
    def __str__(self):
        name = self.guide.get_full_name() or self.guide.username
        return f"{self.get_provider_display()} integration for {name}"
...
class GuideCalendarEvent(models.Model):
    ...
    def __str__(self):
        name = self.integration.guide.get_full_name() or self.integration.guide.username
        return f"{name}: {self.summary or 'External Event'}"
